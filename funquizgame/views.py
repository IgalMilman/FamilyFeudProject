...
from funquizgame.actions.game_actions import (create_game, get_or_create_game,
...
from funquizgame.actions.question_actions import (complete_question,
                                                  ...
                                                  reveal_question)
from funquizgame.actions.user_actions import login_user_access_code, login_user_user_name
...
def homepage(request: HttpRequest):
    return render(request, 'index.html', {'role': request.user.role, 'teamnumber': request.user.team})

@require_http_methods(['GET'])
@login_required(login_url='login')
@ensure_csrf_cookie
def active_game(request: HttpRequest, gameid):
    return render(request, 'index.html', {'role': request.user.role, 'teamnumber': request.user.team, 'gameid': gameid})
...
def authenticate_user_access_code(request:HttpRequest):
    ...
    if user is None or game is None:
        return render(request, 'login.html', {'errormessage': 'Access code is invalid', 'access_code': request.POST.get('access_code', '')})
...
def authenticate_user_username(request:HttpRequest):
    ...
    if user is None:
        return render(request, 'login.html', {'errormessage': 'Username or password are incorrect', 'username': request.POST.get('username', '')})
...
def question_api(request: HttpRequest, gameid, questionid):
    role = RequesterRole.get_role_from_request(request)
    return JsonResponse(get_real_question(role, gameid, questionid), status=200, safe=False)
...
def question_data_api(request: HttpRequest, gameid, questionid):
    role = RequesterRole.get_role_from_request(request)
    return JsonResponse(get_question_data(role, gameid, questionid), status=200, safe=False)
...
def question_get_all_type_api(request: HttpRequest, gameid, questiontype):
    role = RequesterRole.get_role_from_request(request)
    if role.is_host():
        return JsonResponse(get_all_questions_of_type(role, gameid, questiontype), status=200, safe=False)
...
def get_all_active_games_api(request: HttpRequest):
    role = RequesterRole.get_role_from_request(request)
    if role.is_host():
        games = get_all_active_games(role)
        result = {
