...
class QuestionData(MultiLanguageField):
    ...
    def json_short_duplication_check(self, role: RequesterRole, game: Game) -> Dict:
        ...
        result["is_complete"] = (
            False if real_question is None else real_question.is_complete
        )
        return result
    ...
    def from_json(data: Dict, user: GameUser) -> "QuestionData":
        ...
        if id is None:
            question = QuestionData.objects.create(
                question_type=question_type, created_by=user
            )
        else:
            [question, _] = QuestionData.objects.get_or_create(
                unid=id, question_type=question_type
            )
